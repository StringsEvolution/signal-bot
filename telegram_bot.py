...
def send_otc_signal(signal) -> dict:
    ...
    try:
        ...
        um = UserManager()
        # OTC signals are generated by BOTH the Pocket Option and IQ Option
        # engines (both store candles under the same "_otc" codes and flow
        # through this one function). Previously only "pocket_option" users
        # were loaded, so anyone who registered under IQ Option received
        # nothing — the signal fired and settled but was delivered to no one.
        # Load both platforms and de-duplicate by telegram_id (a user who
        # connected both brokers should only get one DM per signal).
        otc_users = []
        seen_ids = set()
        for platform in ("pocket_option", "iq_option"):
            try:
                for u in um.get_all_platform_users(platform):
                ...
                logger.error(
                    f"send_otc_signal: could not load {platform} users — {exc}"
                )
        po_users = otc_users
    except Exception as exc:
        logger.error(f"send_otc_signal: could not load OTC users — {exc}")
        return results

    if not po_users:
        logger.info(
            "send_otc_signal: no OTC users connected (Pocket Option or IQ "
            "Option) — OTC signal not delivered (nothing sent to VIP/Free, "
            "by design)."
        )
        return results
    ...
    logger.info(
        ...
        f"{signal.direction} delivered privately to {len(results)} OTC user(s)."
    )
    return results
